...
fun findNoAllergens(input: List<String>, map: MutableMap<String, Set<String>>): Int {
    ...
    input.forEach { list ->
        val ingredients = list.substringBefore(" (").split(" ")
        ingredients.forEach {
            if(it !in marked) unmarked.add(it)
        }
    }
    return input.sumBy { it.substringBefore(" (").split(" ").toSet().intersect(unmarked).size }
}

fun makeCanonList(map: MutableMap<String, Set<String>>): String = map.keys.sorted().map { map[it]!!.first() }.joinToString(",")
